...
.directive("contenuMail", function() {
	return {
		...
		template: '<div class="spacer"> \
                ...
                <iframe src="about:blank" id="frameEmail"></iframe> \
            </div>',
        scope: {
        	email: "="
        },
        link: function(scope, element, attrs) {
            scope.$watch("email.content", function(content) {
                var items = element.find("iframe");

                if (items.length == 1) {
                    var iframe = items[0];

                    iframe.contentWindow.document.body.innerHTML = content ? content : "";
